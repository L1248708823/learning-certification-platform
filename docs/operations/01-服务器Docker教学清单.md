...
服务器端口只监听本机，本地电脑的 SSH 登录、主机指纹、密码/密钥和端口隧道流程统一记录在
[02-连接服务器.md](02-连接服务器.md) 中。基础设施文档只保留一个边界：本地通过隧道使用
`127.0.0.1:13306`、`127.0.0.1:16379`、`127.0.0.1:19000` 和 `127.0.0.1:19001`；
如果后端最终运行在同一个 Compose 网络内，则使用 `mysql:3306`、`redis:6379`、`minio:9000`。
...
本次 SSH 地址超时、主机指纹和隧道排障记录已独立归档到
[02-连接服务器.md](02-连接服务器.md)，避免服务器更换时同时维护两份连接命令。
